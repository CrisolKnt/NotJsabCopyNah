...
public class LevelEvents : MonoBehaviour
{
    ...
    //objects references to spawn in methods(may remove later)
    GameObject ballexpo;
    GameObject line;
    ...
    private void Awake()
    {
        ...
        AudioSource.time = 118;
        AudioSource.volume = Main_Menu.volumemaster;
   ...
   void line_spawn(float x = 0, float y = 0, float angle = 0, float delay = 0)
    ...
    IEnumerator list_of_moves()
    {
        ...
        yield return new WaitUntil(() => time >= 26.40f); //.25
        shoot_bullet(random_pos());
